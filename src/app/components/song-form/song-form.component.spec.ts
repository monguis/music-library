...
describe('SongFormComponent', () => {
  ...
  beforeEach(async () => {
    mockSongsService = jasmine.createSpyObj('SongsService', [
      'addSongToLocalList',
      'getSongToUpdate',
      'setSongForEdit',
      'updateSong',
    ...
    mockNotificationService = jasmine.createSpyObj('NotificationsService', [
      'pushWarningAlert',
      'pushSuccessAlert',
      'pushErrorAlert',
    ]);
  ...
  describe('On submit behavior', () => {
    ...
    it('should update songs service and notify user on successful update', () => {
      ...
      expect(mockNotificationService.pushSuccessAlert).toHaveBeenCalledWith(
        'Song ID: 1 has been updated successfully'
      );
    ...
    it('should notify the user on update error', () => {
      ...
      expect(mockNotificationService.pushErrorAlert).toHaveBeenCalledWith(
        `Song ID:  could not be updated: ${testErr}`
      );
    });
    ...
    it('should notify the user on create error', () => {
      ...
      expect(mockNotificationService.pushErrorAlert).toHaveBeenCalledWith(
        `New song could not be created: ${testErr}`
      );
    });
  });
